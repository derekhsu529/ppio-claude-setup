...
const htmlContent = fs.readFileSync(f('renderer.html'), 'utf8');
...
const rendererContent = fs.readFileSync(f('renderer.js'), 'utf8');
...
const cssContent = fs.readFileSync(f('styles.css'), 'utf8');
...
const iconPng = fs.statSync(f('assets/icon.png'));
const iconIco = fs.statSync(f('assets/icon.ico'));
const iconIcns = fs.statSync(f('assets/icon.icns'));
...
// 验证 PNG 是 1024x1024（CI 上可能没有 PIL，跳过）
const { execSync } = require('child_process');
try {
  const iconPath = f('assets/icon.png').replace(/\\/g, '/');
  const identify = execSync(`python3 -c "from PIL import Image; img = Image.open('${iconPath}'); print(img.size)"`, { timeout: 5000 }).toString().trim();
  assertEq(identify, '(1024, 1024)', 'icon.png 尺寸 1024x1024');
} catch (e) {
  console.log('  ⏭️  icon.png 尺寸检查跳过（PIL 不可用）');
}
...
const mainContentForMirror = fs.readFileSync(f('main.js'), 'utf8');
...
const rendererContentForMirror = fs.readFileSync(f('renderer.js'), 'utf8');
...
const pkg = JSON.parse(fs.readFileSync(f('package.json'), 'utf8'));
...
const mainContent = fs.readFileSync(f('main.js'), 'utf8');
...
// preload.js: 新 API 暴露
const preloadContent = fs.readFileSync(f('preload.js'), 'utf8');
